...
void setup() {
  ...
  Serial.print(F("Default Ambient Light Sensor gain is:  "));
  Serial.println(veml.getGainValue());
  // According to App Note, if you are waiting for the sensor to refresh at its
  // desired refresh rate, VEML7700_POWERSAVE_MODE1 uses the least ENERGY per
  // sample.
  veml.setPowerSaveMode(
      VEML7700_POWERSAVE_MODE1);    // most power and shortest delay.
  ...
  do {
    ...
    Serial.print(F("  ...  Refresh time for veml is: "));
    Serial.print(refreshTime);
    Serial.print(F(", gain: "));
    Serial.print(veml.getGainValue());
    Serial.print(F(", IntegrationTimeFactor: "));
    Serial.print(veml.getIntegrationTimeFactor());
    Serial.print(F(", PowerSaveMode: "));
    Serial.print(veml.getPowerSaveMode());
    Serial.println();
    delay(refreshTime);
    raw = veml.readALS();
    lux = veml.convertToLux(raw);
    Serial.print(F("Light sample is (raw, lux): "));
    Serial.print(raw);
    Serial.print(F(", "));
    Serial.print(lux);
  } while (veml.optimizeParams(raw));
  Serial.println(
      F("  ... VEML_7700 params optimized for current light conditions."));
...
void loop() {
  ...
  if (optimalParams) {
    if (++loopCount > 20) {
      ...
      Serial.println();
      Serial.print("Setting Power Save Mode to: ");
      Serial.println(currentPowerSaveMode);
    ...
    Serial.print(" ... Setting Power Save Mode to: ");
    Serial.println(currentPowerSaveMode);
    Serial.print(F("New parameters are gain: "));
    Serial.print(veml.getGainValue());
    Serial.print(F(", IntegrationTimeFactor: "));
    Serial.print(veml.getIntegrationTimeFactor());
    Serial.print(F(", PowerSaveMode: "));
  ...
  Serial.print("  ... Wait for VEML to be ready again: ");
